...
use adco_shared::post::{Post, PostStatus};
use sqlx::PgPool;
use uuid::Uuid;

#[derive(Debug, Clone)]
pub struct PostsRepositoryPostgres {
    pool: PgPool,
}


impl PostsRepositoryPostgres {
...
impl PostsRepositoryPostgres {
    ...
    pub async fn get_all_posts(&self) -> Result<Vec<Post>, PostRepositoryError> {
        let result = sqlx::query!("SELECT * FROM posts")
        .fetch_all(&self.pool)
        .await?;
        Ok(result.into_iter().map(|r| Post {
            id: Some(r.id),
...
impl PostRepository for PostsRepositoryPostgres {
